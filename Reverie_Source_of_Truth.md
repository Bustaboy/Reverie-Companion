...
Task 4E adds the Character Encyclopedia as Reverie's living character bible for the active companion. It completes the user-facing Task 4 growth suite by turning Dashboard, Journal, Memory Browser, and Automated LoRA signals into an easy-to-read life summary instead of another administrative panel.

- **Navigation and scope**: the sidebar now includes an `Encyclopedia` destination beside Growth, Journal, Memory, and Training. Growth remains the pulse/dashboard view; Journal remains the diary/evidence view; Memory remains the editable recall library; Training remains Personal LoRA consent and approvals; Encyclopedia is the warm profile synthesis layer.
- **Profile synthesis**: the page derives a local-only active-character overview from existing stores: `journalStore` reflections/insights/themes/confidence, `memoryStore` memory text/metadata/scores, `growthStore` approved examples/job learning feedback, and recent chat TTS voice metadata. It does not run model inference, train adapters, create embeddings, or write durable growth.
- **Character bible sections**: the profile includes how the character sees the user, affection/trust/attraction relationship meters, how they see other important people, living situation and daily routines, likes/dislikes/hobbies/personality quirks, key life experiences, evolution over time, current mood, recent growth highlights, and voice/embodiment notes.
- **Searchable and navigable UI**: the encyclopedia uses a premium warm dark visual style consistent with Growth and Journal, with a hero profile card, visual relationship meters, timeline highlights, section filter chips, keyword search, persistent collapsible sections, rich empty states, and small provenance cues such as local signal counts and confidence percentages.
- **8GB behavior**: Task 4E stays lightweight for RTX 4070 8GB mobile targets. Page load performs only bounded existing API reads already used by the surrounding Task 4 surfaces and small Svelte array/map derivations. No new resident model, GPU job, media generation, background queue, or broad filesystem scan is introduced.
- **Growth-loop completion**: Task 4 now has a coherent transparent growth system: Growth explains what is changing, Journal shows reflective evidence, Memory lets users correct durable recall, Training handles optional approved LoRA adaptation, and Encyclopedia presents the character as a living person with a readable history and current emotional state.
