...
- **VoiceProfile schema** stores `voice_id`, display `name`, `type` (`character` or `narrator`), optional `reference_audio_path`, open-ended `metadata`, and compact `mood_settings` for per-character TTS fine-tuning. The reference path and metadata fields are explicitly future-proofing for zero-shot voice cloning, backend voice aliases, gender/style notes, and other cloning inputs without implementing heavy cloning work in this milestone.
...
- **TTSContext schema** now carries `character_id`, `is_narration`, `mode` (`one_to_one` or `rpg`), `emotion_hint`, `intensity`, optional `scene_tags`, and resolved per-character `mood_settings`. Emotion fields now feed deterministic tag selection and intensity scoring while staying lightweight and inspectable.
...
- **EmotionEngine** is a deterministic, 8GB-friendly service that analyzes `TTSContext`, per-character `mood_settings`, current `scene_tags`, recent non-system chat messages, retrieved memory context, selected reflection journal entries, and growth notifications. It detects high-emotion and intimate/NSFW scenes with bounded keyword scoring, honors `emotion_hint` and `intensity`, applies user-tuned expressiveness/sensitivity/NSFW boosts, and produces Orpheus-compatible speech tags such as `<whisper>`, `<sigh>`, `<gasp>`, `<moan>`, `<groan>`, and `<laugh>`.
...
Task 2G has now completed the advanced mood and final polish layer.

## Milestone 3 Task 2G Update — Per-Character Mood & Final TTS Polish

Task 2G finalizes the Task 2 voice stack into a coherent local-first architecture:

- **Per-character mood controls**: `VoiceProfile` records now persist `mood_settings` with Baseline Expressiveness, Emotional Sensitivity, and NSFW Intensity. The settings UI lets users select a character voice profile, tune those sliders, save them through `PATCH /api/voices/{voice_id}`, and immediately reuse the same profile with cloning, streaming, chat, and VN playback.
- **Mood-aware TTSContext routing**: `TTSContextRouter` enriches every resolved context with the selected voice profile's mood settings before EmotionEngine or TTSService tags speech. This keeps explicit voice overrides, assigned character voices, narrator fallback, and RPG routing centralized while giving emotion/prosody logic character-specific direction.
- **Smarter deterministic scene detection**: EmotionEngine now combines mood settings, scene tags, recent messages, memory context, reflection entries, and growth notifications. High-emotion and intimate/NSFW scenes receive stronger bounded boosts when the user raises sensitivity or NSFW intensity, and calmer settings reduce tag pressure without disabling clean fallback behavior.
- **Clean text remains protected**: visible chat text is still stripped of speech-only tags, `tts_text` remains the only tag-bearing speech script, and final SSE metadata carries the resolved `tts_context`, `voice_id`, and emotion audit cues for frontend playback.
- **AudioPlayer polish**: playback now gives clearer local fallback/error copy, exposes dismiss/reset actions for voice errors, and continues to use the bounded queue, progressive buffer health, interruption behavior, and VN speaking cue from Tasks 2E-2F.
- **8GB-friendly final shape**: all Task 2G logic is Pydantic/TypeScript state and bounded keyword scoring. No classifier, no extra LLM call, no eager Orpheus load, no cloning model residency, and no additional GPU pressure are introduced. Piper remains the CPU fallback, Orpheus remains lazy, and streaming gracefully degrades to buffered full-file playback.

**Final Task 2 architecture summary**: TTSService owns synthesis; VoiceManager owns durable voice identity, cloning references, character assignments, and mood settings; TTSContextRouter resolves narrator/character context plus mood; EmotionEngine produces inspectable speech-only tags from clean deterministic evidence; chat/VN preserve clean visible text while passing `tts_text`, `tts_context`, `voice_id`, and emotion metadata to `ttsStore`; AudioPlayer provides one interruptible local playback surface across chat and VN mode.
