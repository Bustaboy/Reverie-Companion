...
### 3.4 TTS and Voice Profile Foundation (Milestone 3 Tasks 2A–2B)

Milestone 3 Tasks 2A–2B establish Reverie's local-first speech foundation plus a clean voice identity layer:

- **TTSService** owns speech generation workflow behind a typed service boundary. It accepts plain `text`, an optional `voice_id`, an optional `character_id`, requested audio format, and optional streaming mode. When a `VoiceManager` is provided, requests resolve through durable voice profiles before any backend receives a voice identifier.
- **VoiceProfile** is the durable Pydantic schema for voice identity: stable `voice_id`, display `name`, role `type` (`character` or `narrator`), optional `reference_audio_path`, and open-ended `metadata` for future cloning/provider/style information. This stores the data needed for zero-shot voice cloning later without implementing cloning logic now.
- **VoiceManager** is the JSON-backed voice profile service. It loads and persists profiles, creates the default narrator voice when missing, retrieves profiles by ID, resolves narrator fallback, assigns/retrieves character voice mappings, and supports basic create/update/delete operations. The JSON store is intentionally lightweight for MVP/Alpha, keeps startup cheap on 8GB systems, and can be replaced by a richer repository later without changing the TTS boundary.
- **Default voice behavior** is configurable under the `REVERIE_` settings prefix: profile store path, default narrator voice ID/name, and whether unassigned character speech falls back to the narrator or returns no profile. The existing TTS backend settings still control Orpheus/Piper model IDs, paths, device, quantization, timeouts, VRAM guardrails, sample rate, text limits, and stream chunk size.
- **Orpheus TTS 3B** remains the primary quality backend for emotionally richer voice. It is lazily imported and loaded only on the first TTS request, supports `auto`/`cuda`/`cpu` device selection, defaults to 4-bit quantization for RTX 4070 8GB mobile safety, checks free CUDA VRAM before loading, unloads after CUDA OOM paths, and reports stable error codes when dependencies, model paths, or VRAM are unavailable.
- **Piper TTS** remains the fast CPU-friendly fallback. When Orpheus is unavailable, too slow, missing dependencies, missing model files, or over the configured VRAM budget, Reverie attempts Piper through the local binary and configured voice model path.
- **API surface** keeps `POST /api/tts/generate`, returning base64 WAV audio for simple non-streaming clients or bounded audio-byte streaming for early voice playback. The route stays thin, logs request metadata without raw private text, resolves profiles via service dependencies, and returns structured errors for UI handling.

This foundation intentionally avoids Task 2C–2D scope: no emotion tags, no memory/reflection-driven prosody, and no context routing beyond explicit voice/character profile lookup. Future work can layer emotional TTS, cloning, and richer context-aware routing on top of these same service contracts while preserving the 8GB-first lazy-loading and fallback behavior.
