...
Milestone 3 introduces a polished, 8GB-friendly Visual Novel foundation that stays deliberately lightweight:

- **CharacterVisualManifest** defines the character name, versioned defaults, supported expressions, supported poses, backgrounds, full-sprite fallbacks, and ordered layer stacks. Layer composition now resolves from back to front (`base` → `expression` → `clothing` → optional hair/accessory/effect/custom layers) while retaining legacy one-image-per-pose/expression sprite manifests as a safe fallback.
- **Layer assets** support individual images today and optional sprite-sheet `frame` metadata for future atlas/cell rendering. Required layers fall back to full sprites when missing or failed; optional layers are skipped gracefully so broken clothing/accessory art never interrupts chat or immersion.
- **ExpressionManager** normalizes `visual_state` metadata from chat into known expression, pose, and background values, resolves relative manifest asset paths for backgrounds, full sprites, and layers, preserves sprite-sheet frame metadata, and keeps alias handling deterministic. Unknown or missing values always fall back to **neutral + idle + default background** so broken assets never interrupt the companion path.
...
- **visualNovelStore** owns the current resolved visual state, full-immersive toggle, temporary growth modifier, and a bounded failed-asset LRU capped at 8 entries for strict 8GB discipline. Growth cues from final `visual_state` metadata create a 30–60 second modifier with timer-safe replacement semantics that smoothly decays back to the base state while respecting `prefers-reduced-motion`.
- **VisualNovelStage** renders layered character stacks, full-sprite fallbacks, backgrounds, dialogue, aria-live state announcements, keyboard shortcuts (Escape returns to chat; F toggles full immersion), lazy image loading, and warm dark polish. It uses keyed fades, CSS-variable transforms, subtle glows, breathing spacing, reduced-motion guards, and no resident LLM/image-generation/TTS work on the chat path.
- Chat SSE `done` events carry inferred `visual_state` / `visualState` metadata for immediate smooth VN changes after a completed reply. Token `message` frames remain text-only unless future backend metadata explicitly requires otherwise, keeping streaming lightweight.

**Task 1 status**: Milestone 3 Task 1A, 1B, and 1C together complete the Visual Novel Foundation: chat/VN toggling, backend visual-state reactivity, layered rendering, future sprite-sheet readiness, accessibility, polished transitions, bounded asset failure handling, and regression tests. Future tasks should focus on richer authored asset packs, character import tooling, and optional Futa-Vision hooks without adding resident LLMs to the normal chat path.
