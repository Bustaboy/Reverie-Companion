...
class ChatService:
    ...
    def _attach_voice_enhancement(
        ...
        preliminary_text = self._emotion_engine.strip_emotion_tags(
            response.message.content
        )
        tts_context, voice_id = self._resolve_tts_context_and_voice(
            clean_text=preliminary_text, request=request
        )
    ...
    async def _prepare_request_with_growth(
        ...
        *,
        ...
        """Build a model request and choose bounded continuity/reactivity metadata."""
        ...
        # Reflection context supports slow long-term growth on future turns while
        # the current chat stays responsive because new reflection work runs later.
    ...
    def _inject_voice_metadata_into_done_sse(
        ...
        *,
        ...
        preliminary_text = clean_response or self._emotion_engine.strip_emotion_tags(
            raw_response
        )
        tts_context, voice_id = self._resolve_tts_context_and_voice(
            clean_text=preliminary_text, request=request
