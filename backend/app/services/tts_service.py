...
class TTSService:
    ...
    async def generate_speech(
        ...
        *,
        ...
        _, normalized_text, routing = self._prepare_request(
            text=text,
            voice_id=voice_id,
        ...
        raise TTSBackendUnavailable(
            "No local TTS backend is currently available. Text chat is unaffected; check Orpheus/Piper setup or switch to the lightweight Piper path.",
    ...
    def _prepare_request(
        ...
        *,
        ...
        if len(normalized_text) > self._settings.tts_max_text_chars:
            raise TTSServiceError(
                "That voice line is too long for one local TTS request. Play a shorter line or let Reverie speak the start of the reply.",
                code="tts_text_too_long",
                retryable=False,
                details={
                    "max_chars": self._settings.tts_max_text_chars,
                    "actual_chars": len(normalized_text),
                },
            )
