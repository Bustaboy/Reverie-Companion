...
def get_chat_service(
    ...
    """Provide the chat orchestration service for request handlers."""
...
async def chat(
    ...
    The chat service enriches prompts with optional long-term memory before
    Ollama is called. Streaming uses Server-Sent Events with `message` events
    for token chunks, a final `done` event, and an `error` event if Ollama fails
    after the HTTP stream has started.
    """
    ...
    return StreamingResponse(
        chat_service.stream_chat(request, request_id=request_id),
