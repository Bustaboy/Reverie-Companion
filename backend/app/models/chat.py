...
class ChatRequest(BaseModel):
    ...
    character_id: str | None = Field(
        ...
        min_length=1,
        max_length=80,
        description="Optional selected character whose blueprint scopes chat, memory, and growth.",
    )
    ...
    def character_id_must_not_be_blank(cls, value: str | None) -> str | None:
        """Keep character scoping explicit when supplied by clients."""

        if value is not None and not value.strip():
            raise ValueError("Choose a companion before starting this thread.")
        return value
