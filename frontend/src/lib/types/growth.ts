export type PersonalLoRAExampleStatus = 'pending_review' | 'approved' | 'rejected' | 'deleted';
export type PersonalLoRATrainingStatus = 'idle' | 'queued' | 'running' | 'cancelled' | 'completed' | 'failed';
...
export interface PersonalLoRAExample {
  item_id: string;
  ...
  status?: PersonalLoRAExampleStatus;
...
export interface PersonalLoRAJob {
  job_id?: string;
  status?: PersonalLoRATrainingStatus;
...
export interface PersonalLoRAStatusResponse {
  ...
  current_job?: PersonalLoRAJob | null;
